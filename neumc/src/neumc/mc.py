...
def mixture_metropolize(samples_q, log_q, log_p, mixture):
    ...
    for i in range(1, len(samples_p)):
        ...
        if r[i] < torch.exp(log_quot) * (mixture[i-1] / mixture[i]):   # we are sampling with w(\phi_i);
            prev_log_w = log_w[i]
            accepted[i] = 1
            
        else:
            samples_p[i] = samples_p[i - 1]
            samples_p_log_q[i] = log_q[i - 1]  #! should be samples_p_log_q[i] ?
            samples_p_log_p[i] = log_p[i - 1]  #! should be samples_p_log_p[i] ?
...
def metropolize_numpy(
    samples_q, log_q, log_p
):  # same as before, but with numpy arrays;
